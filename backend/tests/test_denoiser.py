# backend/tests/test_denoiser.py
# Denoiser benchmarking tool
#
# Runs both denoiser backends (noisereduce and Facebook DNS64) on all audio files
# in backend/tests/noisy_input/ and writes denoised outputs to
# backend/tests/denoised_output/.
#
# Usage:
#   cd backend
#   python tests/test_denoiser.py
#
# Compare outputs manually to choose the DENOISER config flag in config.py.
...
def denoise_all():
    ...
    for file_path in sorted(audio_files):
        try:
            ...
            print(
                f"  [OK] {file_path.name} | audio: {duration:.2f}s"
                f" | denoised in: {elapsed:.2f}s -> {out_path}"
            )
