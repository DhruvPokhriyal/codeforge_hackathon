...
btnProcess.addEventListener('click', async () => {
  ...
  if (!file) { setStatus('Please select an audio file first.'); return }
...
function renderHITLReport(situations, isVague) {
  ...
  if (isVague) {
    ...
    banner.textContent = '⚠ Vague transcript — expanded with LLM hypotheses'
...
btnApprove.addEventListener('click', async () => {
  ...
  const conditionInput = document.getElementById('manual-condition').value.trim()
  const itemsInput     = document.getElementById('manual-items').value.trim()
  const manualOverride = conditionInput
    ? { condition: conditionInput, items: itemsInput.split(',').map(s => s.trim()).filter(Boolean) }
    : null
...
function renderQueue(queue) {
  queueTbody.innerHTML = queue.map(req => `
    <tr>
...
function renderVolunteers(volunteers) {
  volunteerCards.innerHTML = volunteers.map(vol => {
    const statusClass = `vol-status-${vol.status.toLowerCase()}`
    const countdown   = vol.expected_return ? computeCountdown(vol.expected_return) : null
    const timerHtml   = countdown !== null
      ? `<span class="vol-timer ${countdown < 0 ? 'overdue' : ''}">${formatCountdown(countdown)}</span>`
      : ''
    const returnBtn = vol.status === 'BUSY'
...
function renderInventory(inventory) {
  inventoryTbody.innerHTML = inventory.map(item => {
...
function showReturnModal(volunteerId) {
  const items = prompt(`${volunteerId}: Enter returned items (comma-separated)`, '')
  ...
  window.api.volunteerReturn(volunteerId, returnedItems)
    .then(() => refreshAll())
